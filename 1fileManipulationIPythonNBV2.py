...
The program will get the input file, separate the OtuID from the taxonomic assignment, create an intermediate file where the otuID is in a separate column from the taxonomic ID, then get the table on that file, and extract only the meaningful columns, which will be 1 and 7.
Per information from the Zelezniak_2015 paper, I figured out that the data that  used was the one in the file gg_sample_details_otus_filtered_file.d.03.tsvFinal output file will be otu_event.txt
